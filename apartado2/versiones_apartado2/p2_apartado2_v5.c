...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < N; j++)
        {
            d[i][j] += a[i][0] * (b[0][j] - c[0]);
            d[i][j] += a[i][1] * (b[1][j] - c[1]);
            d[i][j] += a[i][2] * (b[2][j] - c[2]);
            d[i][j] += a[i][3] * (b[3][j] - c[3]);
            d[i][j] += a[i][4] * (b[4][j] - c[4]);
            d[i][j] += a[i][5] * (b[5][j] - c[5]);
            d[i][j] += a[i][6] * (b[6][j] - c[6]);
            d[i][j] += a[i][7] * (b[7][j] - c[7]);
            d[i][j] *= 2;
        }
    }
